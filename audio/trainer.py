...
from dataloader import create_class_weight
...
def inference_mode(model, acouf, qmask, umask, cuda=False):
...
def trainer(args, model, train_loader, valid_loader, test_loader, optimizer, loss_function):
    ...
    for e in range(args.epochs):
        ...
        #         format(e+1, args.epochs, train_loss, train_acc, train_fscore, valid_loss, valid_acc, val_fscore,\
        #                 test_loss, test_acc, test_fscore, round(time.time()-start_time,2)))
    
    train_metrics = {'train_losses': train_losses, 'train_fscores': train_fscores, 'train_accs': train_accs}
    test_metrics = {'test_losses': test_losses, 'test_fscores': test_fscores, 'test_accs': test_accs}
    val_metrics = {'val_losses': val_losses, 'val_fscores': val_fscores, 'val_accs': val_accs}
    
    best_metrics = {'best_loss': best_loss, 'best_fscore': best_fscore, 'best_acc': accuracy_score(best_label,best_pred,sample_weight=best_mask)*100}
    
    metrics = {'train': train_metrics, 'val': val_metrics, 'test': test_metrics, 'best': best_metrics}
